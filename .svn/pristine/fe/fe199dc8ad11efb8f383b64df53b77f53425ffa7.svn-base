            ...
            if(result.ret == '1') {
                ...
                for(var i = 0; i < equipList.length; i++) {
                    ...
                    var div  = "<tr class='text-c'>" +
                        ...
                        "onclick='taskSee(" + data.id +","+data.mouldId+ ")'>详情</a>"+
                        "</td>"+
                        "</tr>";
...
function search() {
    ...
    InitTableNum(0, id, vehicleType,figureNumber,assetCoding,state,confirmState,classConfirmMan); //Load事件，初始化表格数据，页面索引为0（第一页）
    //分页，PageCount是总条目数，这是必选参数，其它参数都是可选
    $("#Pagination").pagination(PageCount, {
        callback: PageCallback, //PageCallback() 为翻页调用次函数。
        prev_text: "« 上一页",
    ...
    //翻页调用
    function PageCallback(index, jq) {
        InitTableNum(index, id, vehicleType,figureNumber,assetCoding,state,confirmState,classConfirmMan);
    }
}
function InitTableNum(pageIndex,id, vehicleType,figureNumber,assetCoding,state,confirmState,classConfirmMan) {
    var pageSize=10;
    var beginIndex = pageIndex*pageSize;
    $.ajax({
        url: "../spotDetail/getDetailMould.do",
...
function taskSee(id,mouldId) {
    var cycle =  Number($("#cycle").val());
    window.location.href="spotCheckMould.html?id="+id+"&cycle="+cycle+"&mouldId="+mouldId;
}
