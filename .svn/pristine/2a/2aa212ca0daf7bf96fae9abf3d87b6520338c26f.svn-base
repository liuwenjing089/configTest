...
function pictureView(url, pictureDiv, pictureVal){
	...
	if(urls != "" && urls.length > 0){
		for(var i=0; i < urls.length; i++){
			...
			var html = "<div>" +
					   	  "<a href='#' onclick=\"view('"+ picturUrl +"')\" style='margin-left:50px;width:100px;' ><span style='border:1px solid #fff;display:inline-block;width:50px;text-align:center;'>图片</span>"+"<span style='display:inline-block;border:1px solid #fff;width:15px;text-align:center;'>"+ (i+1) +"</span>" +"&nbsp;&nbsp;&nbsp;&nbsp;点击查看</a>" +
...
function initFaultComBox() {
	...
	"<div class='formControls col-xs-4 col-sm-4'>"+
		...
		"</select></div>"+"</div>";
	var optionHtml = "<option value='' data-shopList='[]'>请选择</option>";
	$(selectList).each(function (i, e) {
			...
			if(result.ret == '1') {
				...
				if(list!=null) {
					for (var i = 0; i < list.length; i++) {
						...
						//主选框赋值
						if (i % 2 == 0) {
							div += "<div class='row cl'>"+"<label class='form-label col-xs-1 col-sm-1'></label>"
							+"<div class='formControls col-xs-4 col-sm-4'>" +
...
function addPreventiveMaintenance(){
    ...
    layer.open({
        type: 2,
        id: 'mouldRepairOperationForPreventive',
        title: '再发预防报修申请',
        btn: ['提交', '取消'],
        area: [wWidth + 'px', wHeight + 'px'],
        content: 'mouldRepairOperationForPreventive.html?mouldId=' + mouldId + "&reId=" + id, //iframe的url，no代表不显示滚动条
        yes: function (index, layero) {
        	var frameObj = window.document.getElementById('mouldRepairOperationForPreventive').getElementsByTagName('iframe')[0];
...
//    alert("准备中");
//    return;
	Hui_admin_tabs("servicemanPrevention.html?mouldId="+ mouldId + "&reId=" + id, "再发预防报修列表", "repairPrevention");
