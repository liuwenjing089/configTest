...
function query(type,index) {
    ...
    InitTableElse(index); //Load事件，初始化表格数据，页面索引为0（第一页）
        ...
        success: function(result) {
            if(result.ret == '1') {
                ...
                for(var i = 0; i < equipList.length; i++) {
                    ...
                    var div = "";
                    if(loginUser==assman&&(type==3||type==4||type==5)&&data.state==1) {
                    	                   
                        div = "<tr class='text-c' style='background-color: #dbafb2'>";
                        
                    }else if(loginUser!=assman&&(type==3||type==4||type==5)&&data.state==1){
                    ...
                    div +=  "<td>" + j + "</td>" +
                            ...
                            "<td>" +
                            appState(data.state, data.id, data.type, data.detailId, data.workMan, type, data.isKy) +
                            "</td>" +
                            "</tr>";
...
function shimo(id, obj) {
	...
	Hui_admin_tabs("testDie.html?id="+id+ "&passIndex="+ passIndex, "试模记录填写");
}
//填写
function tianxie(id,type,detailId,isKy) {
    if(type == 1){
    	...
    	Hui_admin_tabs("mouldReport.html?id="+id+ "&passIndex="+ passIndex, "填写模具维修记录");
    }
}
//补全信息
function buquan(id,type,detailId) {
    if(type == 1){
    	...
    	Hui_admin_tabs("mouldReportReplenish.html?id="+id+ "&passIndex="+ passIndex, "补全模具维修记录");
    }
}
...
function detail(type, detailId,id,userType, state){
	...
	if(type == 1){
		title = "设备维修详情";
		url = "equipRepairDetail.html?id=" + id+"&userType="+userType + "&passIndex="+ passIndex;
	}else if(type == 2){
		title = "模具维修详情";
		url = "selectMouldReport.html?id=" + id+"&userType="+userType + "&passIndex="+ passIndex;
	}
            ...
            if(result.ret == '1') {
                ...
                for(var i = 0; i < equipList.length; i++) {
                    ...
                    if(userClass == 2){
                        ...
                        "<td>" + checkNull(data.appearance) + "</td>" +
                        "<td>" +
                        appState(data.state, data.id, data.type, data.detailId, data.workMan, userType,data.isKy, data.mouldCssNum) +
                        "</td>" +
                        "</tr>";                        	
                    }else{
...
//点击模具预防再发报修
function preventionOpen(){
	Hui_admin_tabs("servicemanPrevention.html?mouldId=0&reId=", "再发预防报修列表", "repairPrevention");
}