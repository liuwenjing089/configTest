    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.ret == '1') {
                ...
                //确认按钮显示
                if(result.data.state == 3 && userType == 2){
                    document.getElementById("allconfirm").style.display="";
                }else if(result.data.state == 4 && userType == 5){
                    document.getElementById("allconfirm").style.display="";
                }else if(result.data.state == 5 && userType == 6){
                    document.getElementById("allconfirm").style.display="";
                }
                ...
                if(cropMovementValue == 0 || cropMovementValue == null){
                	$("input[name='cropMovement']").removeAttr("checked");
                }
                ...
                if(userType==2 && result.data.state == 3){
                    ...
                    $("#taskConfirmMan").val(result.data.taskConfirmManView);
                    $("#saveConfirmCommander").val(user);
                    $("#saveConfirmCommanderView").val(userName);
                }else if(userType==6  && result.data.state == 5){
                    $("#taskConfirmMan").val(result.data.taskConfirmManView);
                    $("#saveConfirmCommander").val(result.data.saveConfirmCommander);
                    $("#saveConfirmCommanderView").val(result.data.saveConfirmCommanderView);
                    $("#saveConfirmChief").val(user);
                    $("#saveConfirmChiefView").val(userName);
                }else{
                    $("#taskConfirmMan").val(result.data.taskConfirmManView);
                    $("#saveConfirmCommanderView").val(result.data.saveConfirmCommanderView);
                    $("#saveConfirmChiefView").val(result.data.saveConfirmChiefView);
                }
...
function verificationEquip(){
	var flag = true;
	if(equipBean.reason == null || equipBean.reason.trim() == ""){
		layer.alert("原因追究未填写!");
		flag = false;
	}else
	
	if(equipBean.management == null || equipBean.management.trim() == ""){
		layer.alert("处置未填写!");
		flag = false;
	}else
	
	if(equipBean.cropMovement == 0 || equipBean.cropMovement == null){
		layer.alert("是否影响稼动未选择!");
		flag = false;
	}else
	
	if(equipBean.faultType == 0 || equipBean.faultType == null){
		layer.alert("故障分类未选择!");
		flag = false;
	}else
	
	if(equipBean.repairUsetimeType == 0 || equipBean.repairUsetimeType == null){
		layer.alert("维修用时分类未填写!");
		flag = false;
	}
