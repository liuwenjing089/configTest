...
function buquan(id,type,detailId) {
    if(type == 1){
    	...
    	Hui_admin_tabs("mouldReportReplenishPrevention.html?id="+id+ "&passIndex="+ passIndex, "预防再发补全模具维修记录");
    }
}
...
function detail(type, detailId,id,userType, state){
	...
	if(type == 1){
		...
		title = "模具维修详情";
		url = "selectMouldReportPrevention.html?id=" + id+"&userType="+userType + "&passIndex="+ passIndex;
	}
