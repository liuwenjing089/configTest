...
@Service
@Transactional
public class MouldRepairPreventionServiceImpl implements MouldRepairPreventionService {
    @Autowired
	...
	public void insertMouldRepair(MouldRepair mouldRepair) throws Exception{
		//插入维修主表
		RepairPrevention repair = new RepairPrevention();
		
		//获取报修表中报修编号是当天最大的一条
		Map<String, Object> map = repairPreventionDao.selectRepairNum();
		Integer num = null;
		if(map != null){
		...
		repair.setReId(Integer.valueOf(mouldRepair.getReId()));
		...
		repairPreventionDao.insert(repair);
		
		//插入模具维修表
		MouldRepair sql = mouldRepair;
		sql.setReId(repair.getUuid());
	 ...
	 */
	@Override
	public MouldRepair selectMouldRepairById(String id) throws Exception{
		MouldRepair mouldRepair = mouldRepairDao.selectMouldRepairPreventionById(id, Constant.ACTIVE_VALID);
		//把图片uuid转化为图片
...
//	 */
//	@Override
	public void mouldRepairByMonitorConfirm(String id, String userName, int userType) throws Exception{
...
//
	//各级别确认 + 插入模具报修种类
	@Override
	public void mouldRepairByMonitorConfirmAndAddType(String id, String userName, int userType, int mouldId)
			throws Exception {
	...
	//插入模具报修类型表一条新记录
	private void inertMouldPreventionType(String id, String userName, int mouldId) {
		...
		mouldPreventionType.setReId(id);;
